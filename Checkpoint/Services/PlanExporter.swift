...
nonisolated enum PlanExporter {
    ...
    static func markdown(_ ctx: Context) -> String {
        ...
        var lastTicket = ""
        for t in p.tasks {
            if t.ticketKey != lastTicket, Set(p.tasks.map(\.ticketKey)).count > 1 {
                md += "\n### \(t.ticketKey)\n"
    ...
    static func html(_ ctx: Context) -> String {
        ...
        for t in p.tasks {
            ...
            <details class="task" open>
              <summary><input type="checkbox" data-kind="task"\(done ? " checked" : "")><span class="ttl">\(e(t.title))</span>\(t.area.isEmpty ? "" : "<span class=\"chip\">\(e(t.area))</span>")<span class="dot \(t.priority.rawValue)" title="\(t.priority.rawValue) priority"></span></summary>
